...
async function loadBase(){
  ...
  if (accountSettings?.settings?.modo_oscuro !== undefined) applyDarkMode(false);
}
...
function drawSettings(){ const s=accountSettings?.settings||{}; profileContent.innerHTML=`<section class="card"><h2 class="text-3xl font-bold mb-4">Configuración</h2><form id="settingsForm" class="grid gap-3"><label>Nombre<input class="input" name="nombre" value="${h(user.nombre||'')}"></label><label>Teléfono<input class="input" name="telefono" value="${h(user.telefono||'')}"></label><label class="role-card"><input name="modo_oscuro" type="checkbox" ${s.modo_oscuro?'checked':''}> Preferencia visual guardada sin cambiar la interfaz principal</label><p class="field-hint">CommerCity mantiene temporalmente una identidad clara sólida en todas las páginas.</p><h3 class="font-bold mt-3">Preferencias de notificación</h3><div class="grid md:grid-cols-3 gap-2">${['pedidos','pagos','envíos','chat','devoluciones','promociones'].map((k)=>`<label class="role-card"><input type="checkbox" name="pref_${k}" ${s.preferencias_notificaciones?.[k]!==false?'checked':''}> ${h(k)}</label>`).join('')}</div><button class="btn btn-primary" type="submit">Guardar configuración</button></form></section>`; settingsForm.onsubmit=saveSettings; }
async function saveSettings(e){ e.preventDefault(); const prefs={}; ['pedidos','pagos','envíos','chat','devoluciones','promociones'].forEach((k)=>{prefs[k]=settingsForm[`pref_${k}`].checked;}); const body={nombre:settingsForm.nombre.value,telefono:settingsForm.telefono.value,modo_oscuro:settingsForm.modo_oscuro.checked,preferencias_notificaciones:prefs}; await withButtonLoading(settingsForm.querySelector('button'), async()=>{try{const d=await api.patch('/account/settings',body); accountSettings=d; applyDarkMode(false); toast('Configuración guardada');}catch(err){toast(err.message,'error');}}, 'Guardando...'); }

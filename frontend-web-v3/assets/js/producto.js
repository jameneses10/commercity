...
import {shell, bindShell, $, $$, money, toast, h, icon} from './ui.js';
...
function draw() {
  ...
  qty = isOut ? 0 : 1;
  main.innerHTML = `<section class="two-col"><div><img id="mainImg" src="${imgs[0]}" class="w-full rounded-[28px] aspect-square object-cover card" alt="${h(product.nombre)}"><div class="thumbs mt-4">${imgs.map((u, i) => `<img src="${u}" class="${i ? '' : 'active'}" alt="Imagen ${i + 1} de ${h(product.nombre)}">`).join('')}</div></div><aside class="card"><button id="report" class="float-right text-sm muted" type="button">⚑ Reportar</button><span class="pill orange">${h(product.categoria_nombre || 'Producto')}</span><h1 class="text-4xl font-extrabold my-3">${h(product.nombre)}</h1><p class="muted">★ ${avgReviews()} · ${reviews.length || Number(product.total_resenas) || 0} reseñas aprobadas · <b class="${isOut ? 'text-red-600' : 'text-green-600'}">${isOut ? 'Producto agotado' : `Stock ${availableStock}`}</b></p><div class="price my-5">${money(product.precio_final || product.precio)}</div><p class="muted mb-5">${h(product.descripcion || '')}</p><div class="mb-4"><label class="font-bold muted">Cantidad</label><div class="flex items-center gap-3 mt-2"><button id="minus" class="btn btn-secondary" type="button" ${isOut ? 'disabled' : ''}>−</button><input id="qtyInput" class="input text-center max-w-28" type="number" min="${isOut ? 0 : 1}" max="${availableStock}" value="${qty}" ${isOut ? 'disabled' : ''}><button id="plus" class="btn btn-secondary" type="button" ${isOut ? 'disabled' : ''}>+</button></div><p id="stockMsg" class="field-hint">${isOut ? 'No puedes añadir este producto porque no tiene stock disponible.' : `Puedes comprar máximo ${availableStock} unidades.`}</p></div><button id="add" class="btn btn-primary w-full mb-3" ${isOut ? 'disabled' : ''}>${isOut ? 'Producto agotado' : 'Añadir al carrito'}</button><button id="favorite" class="btn ${isFavorite ? 'btn-danger' : 'btn-secondary'} w-full mb-3" type="button">${icon('heart', 'Favorito')} ${isFavorite ? 'Quitar de favoritos' : 'Guardar en favoritos'}</button><button id="chat" class="btn btn-secondary w-full">Chatear con el vendedor</button><div class="mt-6 p-4 rounded-2xl bg-white"><a id="storeLink" class="auth-link" href="tienda.html?id=${Number(product.tienda_id) || ''}"><b>${h(product.tienda_nombre || 'Tienda')}</b></a><p class="text-sm muted"><a href="tienda.html?id=${Number(product.tienda_id) || ''}">Ver perfil público de tienda</a></p></div></aside></section><section class="mt-10 grid lg:grid-cols-[1fr_420px] gap-6"><div><h2 class="text-3xl font-bold mb-4">Reseñas verificadas</h2><div class="grid gap-4">${reviews.map(reviewCard).join('') || '<p class="muted">Sin reseñas aprobadas aún.</p>'}</div></div><form id="reviewForm" class="card grid gap-3"><h2 class="text-2xl font-bold">Escribir reseña</h2><p class="muted">Solo compradores con pedido pagado y envío entregado pueden reseñar. Si no cumples la condición, el backend lo rechazará y mostraremos el motivo.</p><input class="input" name="pedido_id" type="number" min="1" placeholder="Número de pedido" required><input type="hidden" name="producto_id" value="${Number(product.id) || ''}"><label>Calificación<select class="select" name="estrellas" required><option value="5">★★★★★ 5</option><option value="4">★★★★ 4</option><option value="3">★★★ 3</option><option value="2">★★ 2</option><option value="1">★ 1</option></select></label><textarea class="input" name="comentario" rows="4" maxlength="2000" placeholder="Comentario de reseña"></textarea><button class="btn btn-primary" type="submit">Publicar reseña</button><p id="reviewMsg" class="field-hint"></p></form></section>${reportDialogHtml()}`;
  bindGallery(imgs); bindQuantityControls(); add.onclick = addToCart; favorite.onclick = toggleFavorite; chat.onclick = startChat; report.onclick = openReport; cancelReport.onclick = () => reportDialog.close(); reportForm.onsubmit = sendReport; reviewForm.onsubmit = sendReview;
}

async function loadFavoriteState(){
  if(!getToken()) { isFavorite = false; return; }
  try { const d = await api.get('/favorites'); isFavorite = (d.favorites || []).some((f) => Number(f.producto_id) === Number(id)); }
  catch { isFavorite = false; }
}
async function toggleFavorite(){
  if(!getToken()) { toast('Inicia sesión para guardar favoritos','warn'); setTimeout(()=>{location.href='login.html';},900); return; }
  try { if(isFavorite){ await api.delete(`/favorites/${id}`); isFavorite=false; toast('Producto quitado de favoritos'); } else { await api.post(`/favorites/${id}`,{}); isFavorite=true; toast('Producto guardado en favoritos'); } draw(); }
...
function bindQuantityControls(){ plus.onclick=()=>setQty(qty+1); minus.onclick=()=>setQty(qty-1); qtyInput.oninput=()=>setQty(qtyInput.value); qtyInput.addEventListener('change',()=>setQty(qtyInput.value));}

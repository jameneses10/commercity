...
async function render() {
  app.innerHTML = await shell('chat') + `<main class="container"><section class="chat-layout"><aside class="conversation-list"><div class="p-4"><label class="search-pill">⌕<input id="chatSearch" class="bg-white outline-none" placeholder="Buscar chats..."></label></div><div id="list"><p class="p-4 muted">Cargando conversaciones...</p></div></aside><section class="flex flex-col"><header id="chatHead" class="p-4 border-b border-orange-100 font-bold">Selecciona una conversación</header><div id="msgs" class="messages"></div><div id="emojiPanel" class="hidden px-4 py-2 border-t border-orange-100 bg-white">${EMOJIS.map((e) => `<button type="button" class="emoji-choice text-2xl p-2" data-emoji="${e}">${e}</button>`).join('')}</div><form id="msgForm" class="p-4 flex gap-2 border-t border-orange-100"><button type="button" id="emoji" class="btn btn-secondary">😊</button><textarea class="input min-h-12" name="mensaje" placeholder="Escribe un mensaje..."></textarea><input type="file" id="files" name="files" multiple class="hidden"><button type="button" id="attach" class="btn btn-secondary">＋</button><button class="btn btn-primary">Enviar</button></form><div id="filePreview" class="thumbs px-4 pb-3"></div></section></section></main>`;
  bindShell();
...
function messageBubble(m) {
  ...
  return `<div class="bubble ${mine ? 'mine' : ''}"><p>${deleted ? '<i>Mensaje eliminado</i>' : h(m.mensaje || m.contenido || '')}</p>${deleted ? '' : (m.archivos || []).map(fileLink).join('')}<div class="text-xs text-muted-small mt-2">${formatDate(m.created_at)} · ${m.reportado ? 'Reportado · ' : ''}<button class="report" data-id="${Number(m.id) || ''}" type="button">Reportar</button> · <button class="del" data-id="${Number(m.id) || ''}" type="button">Eliminar</button></div></div>`;
}

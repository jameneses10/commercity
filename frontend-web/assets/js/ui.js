...
export async function shell(active = 'home') {
  ...
  const mainLinks = `${navLink({active: active === 'home', key: 'home', href: 'index.html', label: 'Home'})}${navLink({active: active === 'carrito', key: 'carrito', href: 'carrito.html', label: 'Carrito'})}${u ? navLink({active: active === 'pedidos', key: 'pedidos', href: 'perfil.html#orders', label: 'Historial de pedidos'}) : ''}${navLink({active: active === 'chat', key: 'chat', href: 'chat.html', label: 'Chat'})}${u?.rol === 'vendedor' ? navLink({active: active === 'vendedor', key: 'tienda', href: 'vendedor.html', label: 'Tienda'}) : ''}${u?.rol === 'administrador' ? navLink({active: active === 'admin', key: 'admin', href: 'admin.html', label: 'Administración'}) : ''}`;
  const accountLinks = `${navLink({active: active === 'perfil', key: 'perfil', href: 'perfil.html', label: 'Perfil y ajustes'})}<button class="icon-btn nav-icon" id="notifBtn" type="button" aria-label="Notificaciones" title="Notificaciones">${icon('noti', 'Notificaciones')} <span class="badge">${Number(count) || 0}</span></button>${u ? `<button class="icon-btn nav-icon" id="logoutBtn" type="button" aria-label="Cerrar sesión" title="Cerrar sesión">${icon('logout', 'Cerrar sesión')}<span class="sr-only">Cerrar sesión</span></button>` : navLink({active: active === 'login', key: 'login', href: 'login.html', label: 'Ingresar'})}`;
  return `<header class="topbar"><a class="brand brand-lockup" href="index.html" aria-label="CommerCity inicio"><img class="brand-logo" src="assets/img/logo-commercity.png" alt="Logo CommerCity"><span class="brand-text"><span>Commer</span><span>City</span></span></a><nav class="nav" aria-label="Navegación principal"><div class="nav-group nav-main">${mainLinks}</div><div class="nav-group nav-account">${accountLinks}</div></nav></header><div id="notificationsPanel" class="notification-panel hidden" aria-live="polite"></div>`;
}

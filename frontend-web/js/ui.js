...
export function productCard(p){
  ...
  return `<article class="cc-product"><div class="cc-product-media"><img src="${image}" alt="${name}"></div><div class="cc-product-body"><span class="cc-chip">${category}</span><h3 class="text-xl font-bold">${name}</h3><p class="cc-muted">${store}</p><p class="cc-price">${money(price)}</p><div class="cc-product-actions mt-auto"><a class="cc-btn cc-product-view" href="producto-detalle.html?id=${id}">Ver</a><button class="cc-btn cc-product-add" data-cart="${id}"><span class="cc-btn-icon"><img class="cc-icon" src="assets/icons/cc-shopping-cart.svg" alt=""></span>Añadir</button><button class="cc-btn secondary cc-product-favorite" data-favorite="${id}" type="button" aria-label="Agregar a favoritos" title="Agregar a favoritos"><span class="cc-btn-icon"><img class="cc-icon" src="assets/icons/cc-favorites-wishlist.svg" alt=""></span></button></div></div></article>`;
}
